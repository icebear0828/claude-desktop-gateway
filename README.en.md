...
When a released GUI build is launched outside a source checkout, it falls back
to the OS user config directory and creates a default `gateway.local.json` on
first use. The generated config references environment variable names only; it
does not write provider secrets into JSON.

## Desktop Releases

Desktop prereleases are published from Git tags such as `v0.1.0-pre.1`.
The release workflow builds unsigned Wails apps on native runners and uploads:
